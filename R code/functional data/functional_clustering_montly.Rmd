...
we can cluster this with no allignment to spped up the process:
...
let's trty to increase the number of clusters and then possibly joining them later:
...
this is suggesting 3 or 5 clusters:
...
outliers
```{r}
table(df_month[hclustres$memberships==5,2])
```
