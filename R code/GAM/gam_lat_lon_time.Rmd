...
fitting the model of the only space in the ifferent times
...
we can add more informatio about the time:
...
the reml o the other hand is better
...
looking at the district istead:
...
data <- data.frame(district = levels(df_weather_red2$district), effect = re,std.err = se)
```
...
```{r}
saveRDS(gam_day_pol_qp_full2,file='gam_day_pol_qp_full2')
```
